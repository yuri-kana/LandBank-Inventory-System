<nav x-data="{ open: false, notificationOpen: false }" class="bg-gradient-to-r from-green-500 to-emerald-600 shadow-lg fixed top-0 left-0 right-0 z-50">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex items-center">
                    <a href="{{ route('dashboard') }}" 
                       class="inline-flex items-center px-1 pb-1 text-sm font-medium transition-all duration-200 relative group {{ request()->routeIs('dashboard') ? 'text-white' : 'text-green-100 hover:text-white' }}">
                        <i class="fas fa-tachometer-alt mr-2"></i> Dashboard
                        <!-- Active Indicator -->
                        <div class="absolute bottom-0 left-0 right-0 h-0.5 {{ request()->routeIs('dashboard') ? 'bg-yellow-400' : 'bg-transparent group-hover:bg-yellow-300' }} transition-all duration-200"></div>
                    </a>
                    <!-- FIXED: Show correct route based on user role -->
                    <a href="{{ auth()->user()->isAdmin() ? route('admin.items.index') : route('items.index') }}" 
                    class="inline-flex items-center px-1 pb-1 text-sm font-medium transition-all duration-200 relative group {{ (request()->routeIs('admin.items.*') || request()->routeIs('items.*')) ? 'text-white' : 'text-green-100 hover:text-white' }}">
                        <i class="fas fa-boxes mr-2"></i> Inventory
                        <!-- Active Indicator -->
                        <div class="absolute bottom-0 left-0 right-0 h-0.5 {{ (request()->routeIs('admin.items.*') || request()->routeIs('items.*')) ? 'bg-yellow-400' : 'bg-transparent group-hover:bg-yellow-300' }} transition-all duration-200"></div>
                    </a>
                    <a href="{{ route('requests.index') }}" 
                       class="inline-flex items-center px-1 pb-1 text-sm font-medium transition-all duration-200 relative group {{ request()->routeIs('requests.*') ? 'text-white' : 'text-green-100 hover:text-white' }}">
                        <i class="fas fa-clipboard-list mr-2"></i> Requests
                        <!-- Active Indicator -->
                        <div class="absolute bottom-0 left-0 right-0 h-0.5 {{ request()->routeIs('requests.*') ? 'bg-yellow-400' : 'bg-transparent group-hover:bg-yellow-300' }} transition-all duration-200"></div>
                    </a>
                    @if(auth()->user()->isAdmin())
                        <!-- FIXED: Changed from teams.index to admin.teams.index -->
                        <a href="{{ route('admin.teams.index') }}" 
                           class="inline-flex items-center px-1 pb-1 text-sm font-medium transition-all duration-200 relative group {{ request()->routeIs('admin.teams.*') ? 'text-white' : 'text-green-100 hover:text-white' }}">
                            <i class="fas fa-users mr-2"></i> Teams
                            <!-- Active Indicator -->
                            <div class="absolute bottom-0 left-0 right-0 h-0.5 {{ request()->routeIs('admin.teams.*') ? 'bg-yellow-400' : 'bg-transparent group-hover:bg-yellow-300' }} transition-all duration-200"></div>
                        </a>
                    @endif
                </div>
            </div>
                    ...
                    <button class="inline-flex items-center px-4 py-2 border border-yellow-300 rounded-lg text-sm font-medium text-white bg-gradient-to-r from-yellow-500 to-yellow-400 hover:from-yellow-400 hover:to-yellow-300 transition-all duration-200 shadow-md focus:outline-none">
                        <div class="flex items-center">
                            ...
                            <div class="text-left">
                                ...
                                <div class="text-xs text-yellow-100">{{ auth()->user()->isAdmin() ? 'Inventory Head' : 'Staff Member' }}</div>
                            </div>
                            <div class="ml-2">
                    ...
                    <div class="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl py-2 z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-gray-200 overflow-hidden">
                        <div class="px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-green-50">
                            <div class="flex items-center">
                                ...
                                @endif
                                <div>
                                    <div class="font-medium text-emerald-800">{{ Auth::user()->name }}</div>
                                    <div class="text-xs text-emerald-600 truncate">{{ Auth::user()->email }}</div>
                                    @if(auth()->user()->team)
                                        <div class="flex items-center mt-1 text-xs">
                                            <i class="fas fa-users text-yellow-500 mr-1"></i>
                                            <span class="text-emerald-700">{{ auth()->user()->team->name }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="border-t border-gray-100">
                            <a href="{{ route('profile.settings') }}" class="block px-4 py-3 text-sm text-gray-700 hover:bg-emerald-50 hover:text-emerald-700 transition duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gradient-to-r from-emerald-600 to-green-700 shadow-inner mt-16">
        <div class="pt-2 pb-3 space-y-1">
            <a href="{{ route('dashboard') }}" class="block pl-4 pr-4 py-3 text-base font-medium border-l-4 transition-all duration-200 {{ request()->routeIs('dashboard') ? 'border-yellow-400 text-white bg-emerald-700' : 'border-transparent text-green-100 hover:text-white hover:bg-emerald-700' }}">
                <i class="fas fa-tachometer-alt mr-3 text-yellow-300"></i> Dashboard
            </a>
            <!-- FIXED: Show correct route based on user role (Mobile) -->
            <a href="{{ auth()->user()->isAdmin() ? route('admin.items.index') : route('items.index') }}" class="block pl-4 pr-4 py-3 text-base font-medium border-l-4 transition-all duration-200 {{ (request()->routeIs('admin.items.*') || request()->routeIs('items.*')) ? 'border-yellow-400 text-white bg-emerald-700' : 'border-transparent text-green-100 hover:text-white hover:bg-emerald-700' }}">
                <i class="fas fa-boxes mr-3 text-yellow-300"></i> Inventory
            </a>
            <a href="{{ route('requests.index') }}" class="block pl-4 pr-4 py-3 text-base font-medium border-l-4 transition-all duration-200 {{ request()->routeIs('requests.*') ? 'border-yellow-400 text-white bg-emerald-700' : 'border-transparent text-green-100 hover:text-white hover:bg-emerald-700' }}">
                <i class="fas fa-clipboard-list mr-3 text-yellow-300"></i> Requests
            </a>
            @if(auth()->user()->isAdmin())
                <!-- FIXED: Changed from teams.index to admin.teams.index -->
                <a href="{{ route('admin.teams.index') }}" class="block pl-4 pr-4 py-3 text-base font-medium border-l-4 transition-all duration-200 {{ request()->routeIs('admin.teams.*') ? 'border-yellow-400 text-white bg-emerald-700' : 'border-transparent text-green-100 hover:text-white hover:bg-emerald-700' }}">
            ...
            <div class="border-t border-emerald-500 pt-4">
                <a href="{{ route('profile.settings') }}" class="block pl-4 pr-4 py-3 text-base font-medium border-l-4 border-l-yellow-400 text-green-100 hover:text-white hover:bg-emerald-700">
            ...
            <div class="px-4">
                    ...
                    <div>
                        ...
                        @if(auth()->user()->team)
                            <div class="flex items-center mt-1 text-sm">
                                ...
                                <span class="text-green-100">{{ auth()->user()->team->name }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                <div class="mt-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-yellow-400 to-yellow-500 text-white shadow-sm">
                        {{ auth()->user()->isAdmin() ? 'Inventory Head' : 'Staff Member' }}
                    </span>
                </div>
            </div>
