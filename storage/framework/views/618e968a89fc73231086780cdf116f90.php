...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(request()->routeIs('requests.*') || request()->routeIs('notifications.*') || (isset($teamRequest) && $teamRequest)): ?>
<script src="<?php echo e(asset('js/auto-notifications.js')); ?>"></script>
...
document.addEventListener('DOMContentLoaded', function() {
    if (window.autoNotificationSystem) {
        ...
        <?php if(isset($teamRequest) && $teamRequest): ?>
            window.autoNotificationSystem.initForRequest(<?php echo e($teamRequest->id); ?>);
        <?php elseif(request()->routeIs('requests.index')): ?>
            window.autoNotificationSystem.initForAllRequests();
        <?php endif; ?>
        
        // Also auto-mark notifications when viewing notification page
        <?php if(request()->routeIs('notifications.index')): ?>
